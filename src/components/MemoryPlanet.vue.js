...
let nodeWrappers = [];
let coreSprite = null;
let coreWrapper = null;
let raycaster;
let mouse;
let corePulseTween = null;
let meteorCtx = null;
let meteors = [];
...
let planetGeometry;
let orbitGeometry;
let planetTargetPositions;
let orbitTargetPositions;
...
const createVisitedTexture = (color) => {
    const canvas = document.createElement('canvas');
    canvas.width = 64;
    canvas.height = 64;
    const ctx = canvas.getContext('2d');
...
const sphericalToCartesian = (theta, phi, radius) => {
...
const createMemoryNodes = () => {
    props.memories.forEach((memory) => {
        const position = sphericalToCartesian(memory.position.theta, memory.position.phi, PLANET_RADIUS * memory.orbitRadius);
...
const markNodeVisited = (memoryId) => {
    const sprite = nodeSprites.find((s) => s.userData.memory.id === memoryId);
    if (!sprite || sprite.userData.visited)
        return;
    const memory = sprite.userData.memory;
    ...
    gsap.fromTo(sprite.scale, { x: 0.35, y: 0.35 }, { x: VISITED_NODE_SCALE, y: VISITED_NODE_SCALE, duration: 0.5, ease: 'back.out(2)' });
};
...
const applyFormedState = () => {
    const planetPositions = planetGeometry.attributes.position.array;
...
const applyCameraZoomFactor = (factor) => {
    if (!camera || factor <= 0 || Math.abs(factor - 1) <= 0.002)
        return;
    camera.position.z /= factor;
    camera.position.z = THREE.MathUtils.clamp(camera.position.z, MIN_CAMERA_Z, MAX_CAMERA_Z);
};
const applyPinchZoom = (distance) => {
...
const getWheelZoomFactor = (deltaY) => {
...
const handlePointerMove = (event) => {
    ...
    if (props.phase === 'exploring') {
        ...
        const hoverTargets = [...nodeSprites];
        if (coreSprite && isCoreActivated) {
...
const handleClick = (event) => {
    if (props.phase !== 'exploring' || !containerRef.value)
        return;
    ...
    mouse.y = -((event.clientY - rect.top) / rect.height) * 2 + 1;
    raycaster.setFromCamera(mouse, camera);
    const clickTargets = [...nodeSprites];
    if (coreSprite && isCoreActivated) {
...
const initMeteorCanvas = () => {
    if (!meteorCanvasRef.value || !containerRef.value)
        return;
...
const updateAndDrawMeteors = (now) => {
    if (!meteorCtx || !meteorCanvasRef.value || !containerRef.value)
        return;
...
const updateZoomScales = () => {
    if (!camera || props.phase !== 'exploring')
        return;
...
__VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
    ...
    ...{ onPointercancel: (__VLS_ctx.handlePointerUp) },
...
/** @type {__VLS_StyleScopedClasses['memory-planet']} */ ;
...
// @ts-ignore
[handlePointerDown, handlePointerMove, handlePointerUp, handlePointerUp, handlePointerUp, handleWheel, handleClick,];
